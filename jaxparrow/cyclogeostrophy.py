...
def cyclogeostrophy(u_geos: Union[np.ndarray, np.ma.MaskedArray], v_geos: Union[np.ndarray, np.ma.MaskedArray],
    ...
    :param run_inspection: should we evaluate and return the cycloeostrophic imbalance at every step, defaults to False
...
def _iterative(u_geos: np.ndarray, v_geos: np.ndarray, dx_u: np.ndarray, dx_v: np.ndarray,
    ...
    for i in tqdm(range(n_it)):
        ...
        # inspect results
        if run_inspection:
            res_hist[i] = np.histogram(res_np1.flatten(), bins="doane")
            cyclo_l2[i+1] = tools.compute_cyclogeostrophic_diff_jax(u_geos, v_geos, u_cyclo, v_cyclo, dx_u, dx_v,
                                                                    dy_u, dy_v, coriolis_factor_u, coriolis_factor_v)

        if np.all(mask == 1):
            break

    if run_inspection:
        return u_cyclo, v_cyclo, res_hist[:i], cyclo_l2[:i+1]
    else:
...
def _gradient_descent(u_geos: Union[np.ndarray, np.ma.MaskedArray], v_geos: Union[np.ndarray, np.ma.MaskedArray],
    ...
    cyclo_l2 = np.ones(n_it+1) * np.asarray(f(u_cyclo, v_cyclo))
    for i in tqdm(range(n_it)):
        # update x and y using gradient descent
        u_cyclo, v_cyclo, loss_val = _step(f, u_cyclo, v_cyclo, lr)
        if run_inspection:
            cyclo_l2[i+1] = loss_val

    if run_inspection:
        return np.copy(u_cyclo), np.copy(v_cyclo), cyclo_l2
    else:
